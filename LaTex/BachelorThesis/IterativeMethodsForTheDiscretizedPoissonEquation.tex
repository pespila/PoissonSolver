...
\bild{sin1}{10cm}{Dieser Graph stellt den Eigenvektor am ersten Punkt, also $h = \frac{1}{15}$ dar. Es ist eine lange, nicht oszillierende Sinuswelle zu erkennen.}{Eigenvektor mit einer langen, nicht oszillierenden Sinuswelle.}
\bild{sin7}{10cm}{Bei diesen stärker oszillierenden Sinuswellen handelt es sich um den Eigenvektor im Punkt $\frac{7}{15}$.}{Stärker oszillierender Eigenvektor für die eindimensionale Poisson Gleichung.}
\bild{sin13}{10cm}{Die Sinuswellen sind für den Eigenvektor im Punkt $\frac{13}{15}$ kurzwellig und stark oszillierend.}{Stark oszillierender Eigenvektor.}
...
Für die Matrizen $\mat{A}_{i}$ erhalten wir mit $a = 4, b = c = -1$ für die Eigenwerte \\$\lambda_{k} = 4 (1 - \frac{1}{2} \cos(\theta_{k}))$ und die Eigenvektoren $v_{k} = \sin(i\theta_{k})$ für alle $1 \le i,k \le N$.\\
...
Für $1 \le j,l,k \le N$ werden wir nun $\mat{A}_{2D} (v_{k,l})_{j}$ explizit ausrechnen und müssen dafür wieder drei Fälle unterscheiden, bei denen wir die Additionstheoreme benutzen:
\begin{description}
\item[1.] $j = 1$
\begin{eqnarray}
...
Sei $u^{*} \in \mathbb{R}^{n}$ die exakte Lösung des linearen Gleichungssystems $\mat{A}u = f$. Sei weiterhin $u^{k} \in \mathbb{R}^{n}$ die Approximation der Lösung im $k-ten$ Iterationsschritt. Dann gilt für das Residuum:
\begin{equation}
...
In \cite[S. 21]{ALO02} wird gezeigt, wie man den Rechenaufwand verbessern kann, indem man die Struktur von $\mat{A}_{2D}$ optimal ausnutzt.\\
...
Hier nutzen wir nochmals aus, dass $\sin^{2}(x) \in (0,1)$ monoton steigend ist für $x \in (0, \frac{\pi}{2})$. Zudem ist $\cos(x) \in (0,1)$ für $x \in (0,\frac{\pi}{2})$ und $\sin^{2}(\frac{x}{2}) = \frac{1}{2} (1 - \cos(x))$.
...
\bild{omegaeins}{14cm}{Dies ist das Eigenwertspektrum für $\omega = 1$. Das Gesamtschrittverfahren hat keine Glättungseigenschaft. Es liegen nur wenige der Eigenwerte um die Null (dunkelrote Fläche).}{Eigenwertspektrum der Jacobi Iterationsmatrix, bzw. der Iterationsmatrix des Jacobi-Relaxationsverfahrens mit Parameter $\omega = 1$.}

Da die $n$ Eigenvektoren $(v_{k,l})_{i,j}$ für $1 \le i,j,k,l \le N$, wobei $k,l$ beliebig, eine Basis bilden, lässt sich der Fehler $e$ als Linearkombination der $(v_{k,l})_{i,j}$ darstellen:
\begin{eqnarray}
e^{k+1} &=& \sum_{i = 1}^{N} \sum_{j = 1}^{N} \xi^{k+1}_{i,j} (v_{k,l})_{i,j} = (Id - \omega D^{-1}A) \left(\sum_{i = 1}^{N} \sum_{j = 1}^{N} \xi^{k}_{i,j} (v_{k,l})_{i,j}\right) \notag \\
&=& \left(\sum_{i = 1}^{N} \sum_{j = 1}^{N} \xi^{k}_{i,j} (Id - \omega \underbrace{D^{-1}}_{=\frac{1}{4} Id} A) (v_{k,l})_{i,j}\right) = \left(\sum_{i = 1}^{N} \sum_{j = 1}^{N} \xi^{k}_{i,j} ((v_{k,l})_{i,j} - \frac {\omega} {4} \underbrace{A (v_{k,l})_{i,j}}_{= \lambda_{i,j}(A)(v_{k,l})_{i,j}})\right) \notag \\
&=& \left(\sum_{i = 1}^{N} \sum_{j = 1}^{N} \xi^{k}_{i,j} \underbrace{(1 - \frac {\omega} {4} \lambda_{i,j}(A))}_{= \lambda_{i,j}(\mat{T}_{J_{\omega}})} (v_{k,l})_{i,j}\right) = \left(\sum_{i = 1}^{N} \sum_{j = 1}^{N} \xi^{k}_{i,j} \lambda_{i,j}(\mat{T}_{J_{\omega}}) (v_{k,l})_{i,j}\right).
\end{eqnarray}

\bild{omegaeinhalb}{14cm}{Für $\omega = \frac{1}{2}$ ist gut zu erkennen, dass für $i,j$ nahe $N$ viele Eigenwerte nahe Null liegen (schwarze Fläche).}{Eigenwertspektrum der Iterationsmatrix des Jacobi-Relaxationsverfahrens mit dem Parameter $\omega = \frac{1}{2}$.}\label{img.Jacobi2}
...
Es stellt sich z.B. heraus, dass der optimale Relaxationsparameter, der unabhängig von der Schrittweite $h$ gewählt werden kann, $\omega = \frac {4} {5}$ ist \cite[S. 416]{SAAD03}. In \subfigureautorefname{ 3.3} ist gut zu sehen, dass ein Großteil des Spektrums nahe Null liegt.
Betrachten wir nun die Eigenwerte der Iterationsmatrix für $\omega = 1$ (Jacobi-Verfahren), sieht man, dass die Eigenwerte für $i,j$ nahe Null oder $i,j$ nahe N den Fehler schlecht bis gar nicht dämpfen (\subfigureautorefname{ 3.1}). Sie liegen nahe 1 bzw. -1 und verändern die zugehörigen Eigenvektoren kaum.\\
Das Jacobi-Verfahren besitzt wegen seines Eigenwertspektrums keine Glättungseigenschaft. Als iteratives Verfahren jedoch stellt $\omega = 1$ den optimalen Parameter dar \cite[S. 415]{SAAD03}.\\
...
An dieser Stelle soll noch ein kurzer Satz über die Konvergenz des Verfahrens folgen. Einen Beweis zur Konvergenz findet man u.a. in \cite{GL96}.
...
\subsection{Algorithmus der vorkonditionierten konjugierten Gradienten}\label{ss.Algorithmus PCG}
...
Es bleiben zwei Fragen unbeantwortet:
\begin{description}
...
\bild{V-cycle}{6cm}{\textbf{rot:} a priori Glättung, \textbf{gelb:} Restriktion, \textbf{grün:} direktes Lösen, \textbf{braun:} Prolongation, \textbf{blau:} a posteriori Glättung und \textbf{schwarz:} a posteriori Glättung und Korrektur des Fehlers.}{Veranschaulichung eines V-Zyklus in den Mehrgittermethoden.} \label{V-Cycle}
...
\bild{W-cycle}{12cm}{\textbf{rot:} a priori Glättung, \textbf{gelb:} Restriktion, \textbf{grün:} direktes Lösen, \textbf{braun:} Prolongation, \textbf{blau:} a posteriori Glättung und \textbf{schwarz:} a posteriori Glättung und Korrektur des Fehlers.}{Ein W-Zyklus der die einzelnen Schritte illustriert.} \label{W-Cycle}
...
\bild{MyFunctionOnSquare}{12cm}{Dieser Graph entspricht der Lösung der Poisson-Gleichung auf dem Einheitsquadrat mit $h = \frac{1}{1024}$. Die Lösung wurde durch einen V-Zyklus mit 3 Gittern bestimmt.}{Plot des Lösungsvektors nach einem V-Zyklus im Einheitsquadrat.}
