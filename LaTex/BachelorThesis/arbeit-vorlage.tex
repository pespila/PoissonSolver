...
Zunächst wollen wir nun den zentralen Differenzenquotienten (zweiter Ordnung) einführen. Diesen verwenden wir bei der Diskretisierung unseres Gitters $\Omega$. Die Matrix, die wir für den Fall der Poisson Gleichung erhalten werden, wird anschließend auf ihre Eigenschaften untersucht.
...
Löst man nun \autoref{eq.Partiall x+h} und \autoref{eq.Partiall x-h} jeweils nach $\partial_{xx} u(x,y)$ auf, wobei der Term dritter Ordnung ignoriert wird und addiert diese, so erhält man:
...
\bild{diffStar}{8cm}{5-Punkt-Differenzenstern im Gitter, wobei für alle $1 < i,j < N$ jeder Punkt $(x,y) \in \Omega_{h}$ genau vier Nachbarn hat in $\Omega_{h}$ besitzt.}{5-Punkt-Differenzenstern im Gitter, wobei für alle $1 < i,j < N$ jeder Punkt $(x,y) \in \Omega_{h}$ genau vier Nachbarn hat in $\Omega_{h}$ besitzt.}\label{img.5-Point-Star}

Mit \autoref{eq.Differenzenquotienten} fassen wir $\Delta u(x,y) = \Delta_{h} u(x,y)$ in diskretisierter Form auf, mit:
\begin{equation}
...
Nun wissen wir, wie $-\delta u_{h}$ in Matrixform aussieht. Die rechte Seite $f$ der partiellen Differentialgleichung kann nun ebenfalls in diskretisierter Form $f_{h}$ geschrieben werden. Zu jeder Komponente von $f_{h}$, die einen Randpunkt als Nachbarn hat, wird dieser dazu addiert. Hat eine Komponente von $f_{h}$ zwei Nachbarn am Rand von $\Omega_{h}$, werden beide addiert. Dies führt uns auf folgende rechte Seite, wobei wir diese nun als $f$ statt als $f_{h}$ auffassen wollen [Dahmen/Reusken]:
...
Dies führt nun auf das lineare Gleichungssystem der Form $\mat{A}u = f$, wobei $\mat{A}$ der diskretisierte Laplace-Operator ist, $f$ die rechte Seite und $u$ die approximierte Lösung, der partiellen Differentialgleichung darstellen.\\
Wir bezeichnet ab jetzt die diskrete 2D Poisson Matrix aus \autoref{eq.2D Matrix} mit $\mat{A}_{2D}$. \\
Ab jetzt soll stets $h = \frac{1}{m}$ die Schrittweite sein, wobei $m \in \mathbb{N}_{m > 1}$. Zudem setzen wir $N = m-1$ und $n = N^{2}$, so dass wir die Poisson Matrix als $\mat{A}_{2D} \in \mathbb{R}^{n \times n}$ auffassen werden.

\section{Eigenschaften der Matrix $\mat{A}_{2D}$}\label{s.Eigenwerte und Eigenvektoren}

Es gilt $\mat{A}_{2D} = \mat{A}_{2D}^{T}$. Also ist $\mat{A}_{2D} \in \mathbb{R}^{n \times n}$ eine symmetrische Matrix. Somit existiert eine Orthogonalbasis aus Eigenvektoren für die gilt:
\begin{equation}
...
Für $k \in \mathbb{N}$ und $h = \frac {1} {m}$, seien $(x_{k},y_{l}) \in \Omega_{h}$ mit $x_{k} := k \cdot h, y_{l} := l \cdot h$ und $\theta_{k} \in \mathbb{R}$ mit $\theta_{k} := k \pi h$. Dann gilt für die Eigenwerte:
\begin{equation}
...
Um sich die Eigenvektoren besser vorstellen zu können, wollen wir zunächst drei Eigenvektoren des eindimensionalen Poisson Problems betrachten. Es wird das Gebiet $\Omega_{h} = (0,1)$ diskretisiert und es gilt $u(x)' = g(x)$ mit Randbedingung $u(x) = f(x)$. Dann erhält man auf einem Gitter $(0,1)$ mit $h = \frac{1}{m}$, die Eigenvektoren in den Punkten 1,7 und 13:

\bild{sin1}{8cm}{Dieser Graph stellt den Eigenvektor am ersten Punkt, also $h = \frac{1}{15}$ dar. Zu sehen sind harmonische, kurzwellige Sinuskurven.}{Für Faktor 1}
...
\bild{sin13}{8cm}{Diese langen Sinuswellen gehören zum Punkt $\frac{13}{15}$.}{Für Faktor 13}\label{img.1D Langwelle}
...
\bild{EV}{14cm}{Es sind die Sinuswellen des Eigenvektors im Gitterpunkt $(\frac {3} {5},\frac {3} {5})$ zu sehen. Sie sind langwellig und nur schwach oszilierend.}{Man kann am Eigenvektor des Gitterpunktes $(\frac {3} {5},\frac {3} {5})$ gut die überlagerten Sinuswellen erkennen.}\label{img.Jacobi1}
...
\end{equation}
Nun wollen wir diese Erkenntnisse für die Matrix $\mat{A}_{2D} \in \mathbb{R}^{n \times n}$ verwenden. Da diese Matrix ebenfalls eine Tridiagonalmatrix ist, folgt für $a = A_{j}$ und $b = c = -Id$.\\
...
\item[1.] $j = 1$
\begin{eqnarray}
&&\underbrace{A_{1} v_{k}}_{=\lambda_{k} v_{k}} \sin(\theta_{l}) + (\underbrace{-Id v_{k}}_{\mu_{k} v_{k}}) \underbrace{\sin(2\theta_{l})}_{2\cos({\theta_{l})\sin({\theta_{l}}) = \sin(\theta_{k} + \theta_{l})}} \notag \\
...
&=& \underbrace{(2(1 - \frac{1}{2}\cos(\theta_{k})) + 2(1 - \frac{1}{2}\cos(\theta_{l}))}_{\textnormal{mit } 2 - \cos(x) = \sin^{2}(\frac{x}{2})} = 4 \left(\sin^{2}(\frac {\theta_{k}} {2}) + \sin^{2}(\frac {\theta_{l}} {2})\right)\notag \\
&=& \lambda_{k,l} v_{k,l} \notag
\end{eqnarray}
...
Ein iteratives Verfahren mit beliebigen Startvektor $u^{0} \in \mathbb{R}^{n}$ konvergiert genau dann gegen die exakte Lösung $u^{*} \in \mathbb{R}^{n}$, wenn gilt:
\begin{equation}
...
Dabei sind $\mat{D},\mat{L},\mat{U} \in \mathbb{R}^{n \times n}$, wobei $\mat{D} = diag(a_{1,1},...,a_{n,n})$, $\mat{L}$ eine strikte untere und $\mat{U}$ eine strikte obere Dreiecksmatrix ist. \\
...
Wir können für das Jacobi-Verfahren zwar die Konvergenz nicht verbessern, allerdings ist es möglich den Rechenaufwand zu schmälern. Dies gelingt uns, in dem wir die Dünnbesetztheit von $\mat{A}_{2D}$ gezielt ausnutzen. Es sind pro Zeile maximal $5$ Einträge ungleich Null. Oder anders formuliert, hat jeder Gitterpunkt in $\Omega_{h}$ höchstens 4 Nachbarn. Nutzt man diese Struktur aus, so erhält man den folgenden Algorithmus. 
...
\end{eqnarray}
\autoref{eq.Jacobi-Relaxation} stellt die Iterationsvorschrift für das Jacobi-Relaxationsverfahren dar. Die Iterationsmatrix ist offensichtlich gegeben durch:
\begin{equation}
...
Mit der selben Vorgehensweise wie in \autoref{ss.Jacobi-Verfahren der Poisson Gleichung} verbessern wir noch den Rechenaufwand bzgl. $\mat{A}_{2D}$.
...
\bild{omeganullacht}{14cm}{Für den optimalen Parameter $\omega = \frac{4}{5}$ ist gut zu erkennen, dass viele Eigenwerte um die Null liegen (orange Fläche) und somit eine gute Glättungseigenschaft besteht.}{}\label{img.Jacobi3}
...
\bild{startError}{12cm}{Die Oszillation der Sinuswellen im Startfehler ist hier gut zu erkennen.}{Die Oszillation des Startfehlers ist hier schön zu erkennen.}
...
\bild{oneStepError}{12cm}{Nach einem Iterationsschritt ist bereits eine Glättung des Fehlers zu beobachten.}{Nach einem Iterationsschritt ist bereits eine Glättung des Fehlers zu beobachten.}
\bild{threeStepError}{12cm}{Man sieht, dass bereits nach 3 Iterationsschritten der Fehler sehr glatt ist.}{Die Oszillation im Fehlerterm ist nahezu verschwunden.}
...
Die Matrizen $\mat{A}_{2D_{ih}}$ sind also für alle $i \in \{x^{n} \in \mathbb{N} | n \in \mathbb{N} \}$ stets bekannt.
...
\bild{V-cycle}{6cm}{Text über V-Zyklus}{Text über V-Zyklus}\label{img.V-cycle}
...
\bild{W-cycle}{8cm}{Text über W-Zyklus}{Text über W-Zyklus}\label{img.W-cycle}
...
\bild{MyFunction}{12cm}{Die analytische Lösung für diese Poisson-Gleichung war gegeben durch $u(x,y) = x^{2} + y^{2}$.}{Die analytische Lösung für diese Poisson-Gleichung war gegeben durch $u(x,y) = x^{2} + y^{2}$.}
...
\bild{MyFunctionOnSquare}{12cm}{Dieser Graph entspricht der Lösung der Poisson-Gleichung auf dem Einheitsquadrat. Als Ausgangsdaten wurde die Lösung durch einen Mehrgitter-V-Zyklus verwendet mit $m = 1024$.}{Dieser Graph entspricht der Lösung der Poisson-Gleichung auf dem Einheitsquadrat. Als Ausgangsdaten wurde die Lösung durch einen Mehrgitter-V-Zyklus verwendet mit $m = 1024$.}
...
\multirow{5}* & Schritte & 1340 & 5344 & 21341 & 85282 \\\cline{2-6}
...
\begin{tabular}{|ll||l|l|l|l|}\hline
\multicolumn{2}{|c||}{$m$} & 32  & 64 & 128 & 256 \\\hline\hline
\multirow{5}* & Schritte & 1676  & 6681 & 26676 & 106603 \\\cline{2-6}
...
\multirow{7}* & Schritte & 16  & 32 & 63  & 126 & 251 & 502 \\\cline{2-8}
...
\begin{tabular}{|ll||l|l|l|l|l|l|}\hline
\multicolumn{2}{|c||}{$m$} & 32  & 64 & 128 & 256 & 512 & 1024 \\\hline\hline
\multirow{7}* & Schritte & 7  & 9 & 12  & 17 & 24 & 33 \\\cline{2-8}
...
\multirow{9}* & Schritte & 3  & 3 & 3  & 3 & 3 & 3 & 3 & 3 \\\cline{2-10}
...
\begin{tabular}{|ll||l|l|l|l|l|l|l|l|}\hline
\multicolumn{2}{|c||}{$m$} & 32  & 64 & 128 & 256 & 512 & 1024 & 2048 & 4096 \\\hline\hline
\multirow{9}* & Schritte & 5  & 4 & 4  & 4 & 4 & 4 & 4 & 4 \\\cline{2-10}
...
\begin{tabular}{|ll||l|l|l|l|l|l|l|l|}\hline
\multicolumn{2}{|c||}{$m$} & 32  & 64 & 128 & 256 & 512 & 1024 & 2048 & 4096 \\\hline\hline
\multirow{9}* & Schritte & 5  & 5 & 5  & 4 & 4 & 4 & 4 & 3 \\\cline{2-10}
