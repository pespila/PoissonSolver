...
Die folgenden Aussagen sind weitestgehend in \cite{DAR1} wieder zu finden.\\
...
Aus \cite{SAA1} für $k,l \in \mathbb{N}$ sei $(x_{k},y_{l}) \in \Omega_{h}$ mit $x_{k} := k \cdot h, y_{l} := l \cdot h$. Zudem sind $\theta_{k}, \theta_{l} \in \mathbb{R}$ mit $\theta_{k} := k \pi h$ bzw. $\theta_{l} := l \pi h$. Dann gilt für die Eigenwerte:
\begin{equation}
...
\begin{equation}
(v_{k,l})_{i,j} = \sin(i \pi x_{k}) \sin(j \pi y_{l}) = \sin(i\theta_{k}) \sin(j\theta_{l}) \textnormal{ für } 1 \le i,j \le N.
...
In \cite{DAR2} wird gezeigt, dass für die Kondition der Matrix $\mat{A}_{2D}$ gilt:
\begin{equation}
...
$\blacksquare$
\end{flushright}
In \cite{DAR3} wird außerdem gezeigt, dass sich $\kappa_{2}(\mat{A}_{2D})$ wie folgt nähern lässt:
...
Ein Großteil der Definitionen, Sätze und Lemmata in diesem Kapitel sind sinngemäß aus \cite{DAR4}. Der Abschnitt über das Jacobi-Relaxationsverfahren wurde in Teilen von \cite{SAA2} übernommen. Stellen, an denen andere Literatur verwendet wurde, sind deutlich gekennzeichnet.
...
Einen Beweis hierzu findet man z.B. in \cite{DAR5}.
...
Dieser Abschnitt wurde sinngemäß aus [NumParVer S. 21] übernommen. Der Algorithmus wurde auf unser Problem hingehend optimiert.
...
Es stellt sich z.B. heraus, dass der optimale Relaxationsparameter, der unabhängig von der Schrittweite $h$ gewählt werden kann, $\omega = \frac {4} {5}$ ist \cite{SAA4}. In \subfigureautorefname{ 3.3} ist gut zu sehen, dass ein Großteil des Spektrums nahe Null liegt.
...
Einen kurzen Beweis hierzu findet man z.B. in \cite{DAR6}.\\
...
Der Beweis zu diesem Lemma wird in \cite{DAR7} erläutert.
...
Für einen Beweis möchte ich auf \cite{DAR8} verweisen.
...
Wir wollen ohne Herleitung und Beweis (siehe z.B. [Dahmen/Reusken]) angeben, wie man die $p^{k}$ effizienter bestimmen kann.
...
Substituiert man geschickt einige Werte in den Skalarprodukten (ohne Beweis [DaR]), führt das auf den Algorithmus der konjugierten Gradienten.
