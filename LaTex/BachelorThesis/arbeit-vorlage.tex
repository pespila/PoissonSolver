...
Um sich die Eigenvektoren besser vorstellen zu können, wollen wir zunächst drei Eigenvektoren des eindimensionalen Poisson Problems, auf das wir hier nicht näher eingehen möchten, betrachten:

\bild{sin1}{8cm}{Für Faktor 1}{Für Faktor 1}
\bild{sin7}{8cm}{Für Faktor 7}{Für Faktor 7}
\bild{sin13}{8cm}{Für Faktor 13}{Für Faktor 13}

Man sieht deutlich, das die Oszillation unterschiedlich ist. Manche der Eigenvektoren sind langwellig, manche enorm kurzwellig. Wenn wir nun zurück zu unserer Ausgangssituation des zweidimensionalen Poisson Problems gehen und das Gitter aus \autoref{img.gridWithNumbers} als Basis nehmen, erhalten wir für den Punkt 11 des Gitters folgende Darstellung des Eigenvektors:

\bild{EV}{14cm}{Man kann am Eigenvektor des Gitterpunktes $(\frac {3} {5},\frac {3} {5})$ gut die überlagerten Sinuswellen erkennen.}{Man kann am Eigenvektor des Gitterpunktes $(\frac {3} {5},\frac {3} {5})$ gut die überlagerten Sinuswellen erkennen.}\label{img.Jacobi1}
...
Mit $\sin^{2}(x) \in [0,1]$ für $x \in [0,p\i]$ und $0 < \frac{\pi h}{2} < \pi$ gilt:
\begin{equation}
\lambda_{k,l} = 4 \left(\sin^{2}(\frac {\theta_{k}} {2}) + \sin^{2}(\frac {\theta_{l}} {2})\right) > 0.
\end{equation}
Zudem gilt wegen $h = \frac{1}{m}$:
\begin{eqnarray}
...
Aus \autoref{eq.Lambda min} und \autoref{eq.Lambda max} folgt:
\begin{equation}
...
Für ein großes $m$, also ein feines Gitter, wird die Kondition sehr groß. Dies führt uns auf ein schlecht konditioniertes Problem.
...
\bild{interpolation}{12cm}{Bei der Interpolation bedienen sich die Punkte vom feinen Gitter, den gewichteten Punkten des groben Gitters.}{Bei der Interpolation bedienen sich die Punkte vom feinen Gitter, den gewichteten Punkten des groben Gitters.}
...
\bild{restrict}{12cm}{Ausgehend von einem Punkt innerhalb des Gitters, ist hier die Gewichtung der Werte schön veranschaulicht.}{Ausgehend von einem Punkt innerhalb des Gitters, ist hier die Gewichtung der Werte schön veranschaulicht.}\label{img.restrict}
