...
\usepackage{geometry}
\geometry{tmargin=3cm,bmargin=3cm,lmargin=5cmm,rmargin=2cmm}
...
Zunächst wollen wir nun den zentralen Differenzenquotienten (zweiter Ordnung) einführen. Diesen verwenden wir dann bei der Diskretisierung unseres Gitters $\Omega$. Die Matrix, die wir für den Fall der Poisson Gleichung erhalten werden, soll dann auch noch auf ihre Eigenschaften untersucht werden.
...
\begin{equation}
-\Delta u(x,y) \approx \frac {u(x-h,y) - u(x+h,y) + 4u(x,y) - u(x,y-h) - u(x,y+h)} {h^{2}}.\label{eq.Differenzenquotienten}
\end{equation}
...
Mit \autoref{eq.Differenzenquotienten} fassen wir $\Delta u(x,y) = \Delta_{h} u(x,y)$ in diskretisierter Form auf:
\begin{equation}
-\Delta_{h} u(x,y) = \frac {u(x-h,y) - u(x+h,y) + 4u(x,y) - u(x,y-h) - u(x,y+h)} {h^{2}}.\label{eq.5-Point-Star}
\end{equation}
für alle $(x,y) \in \Omega_{h}$. \\
Man stellt $\Delta_{h} u(x,y)$ auch als 5-Punkt-Differenzenstern (\autoref{img.5-Point-Star}) in der Form
\begin{equation}
...
Es werden nun alle Gitterpunkte des Gitters fortlaufend von links unten nach rechts oben (\autoref{img.gridWithNumbers}) nummeriert. Stellt man nun \autoref{eq.5-Point-Star} für jeden Punkt auf, so erhält man eine $N^{2} \times N^{2}$-Matrix. In dieser ist der 5-Punkt-Differenzenstern gut zu erkennen. Offensichtlich werden bei jedem Punkt des Gitters, der mindestens einen Randpunkt als Nachbarn hat, in der Matrix Werte unterdrückt. Diese tauchen später in der rechten Seite des Gleichungssystems wieder auf.
...
\chapter{Implementierung und Beispiele}\label{c.Vergleich}
...
\bild{MyFunction}{10cm}{Die analytische Lösung für diese Poisson-Gleichung war gegeben durch $u(x,y) = x^{2} + y^{2}$.}{Die analytische Lösung für diese Poisson-Gleichung war gegeben durch $u(x,y) = x^{2} + y^{2}$.}
...
\subsection{Jacobi-Relaxation C++-Methode}\label{s.cpu}\lstinputlisting[language=C]{code/JacobiRelax.c}
...
\begin{tabular}{|ll||l|l|l|l|}\hline
\multicolumn{2}{|c||}{$N$} & 40  & 80 & 160 & 320 \\\hline\hline
\multirow{5}* & Schritte & 4186  & 16693 & 66667  & 266456  \\\cline{2-6}
...
Der C++-Code für das CG-Verfahren, wie auch im nächsten Abschnitt das PCG-Verfahren ist verhältnismäßig lang und würde den Rahmen dieses Kapitels sprengen. Die Implementierung folgt aber eins zu eins dem Algorithmus aus \autoref{ss.Numerisches CG} und \autoref{ss.Algorithmus PCG}.
...
\multirow{5}* & Schritte & 13  & 12 & 12  & 11 \\\cline{2-6}
...
\begin{tabular}{|ll||l|l|l|l|}\hline
\multicolumn{2}{|c||}{$N$} & 32  & 64 & 128 & 256 \\\hline\hline
\multirow{5}* & Schritte & 11  & 11 & 10  & 9  \\\cline{2-6}
