...
Um nun auf ein lineares Gleichungssystem der Form $\mat{A}u = f$ zu kommen, muss natürlich noch die rechte Seite, also das $f$ aufgestellt werden. Zu jeder Komponente in $f$, die einen Randpunkt als Nachbarn hat, wird dieser dazu addieret. Dies führt uns auf folgende rechte Seite:
...
\section{Eigenwerte und Eigenvektoren der Matrix $\mat{A}_{2D}$}\label{s.Eigenwerte und Eigenvektoren}

Die Kondition der Matrix $\mat{A}_{2D}$ spielt in den iterativen Verfahren, wie auch bei den Mehrgittermethoden eine große Rolle. Sie entscheidet über die Konvergenzeigenschaften und die Effizienz des Algorithmus.\\
Erinnerung: Da $\mat{A}_{2D}$ symmetrisch ist, existiert eine Orthogonalbasis aus Eigenvektoren für die gilt:
\begin{equation}
\mat{A}_{2D}x = \lambda x
\end{equation}

\subsection{Eigenvektoren von $\mat{A}_{2D}$}\label{ss.Eigenvektoren}

Die Eigenvektoren für die Matrix $\mat{A}_{2D}$ aus \autoref{eq.PoissonMatrix} lassen sich in folgender Form darstellen:
\begin{equation}
...
Für die Eigenwerte der Matrix $\mat{A}_{2D}$ gilt:
\begin{equation}
...
\end{equation}
wobei $(n-1)$ die Anzahl der Gitterpunkte in x- und in y-Richtung definiert.

Für das Lemma in \autoref{ss.Matrixkondition} ist folgende Definition hilfreich:
...
Da $\mat{A}_{2D}$ s.p.d. ist, gilt mit \autoref{eq.Kondition} $\kappa_{2} (A_{2D}) = \frac {\lambda_{max}} {\lambda_{min}}$. Für die Eigenwerte gilt aus \autoref{eq.Eigenwerte}:
\begin{equation}
\lambda_{ij} = \frac {4} {h^{2}} \left(\sin^{2}(\frac {i \pi h} {2}) + \sin^{2}(\frac {j \pi h} {2}) \right) \textnormal{ für } 1 \le i,j \le n-1
\end{equation}
Dann folgt:
\begin{equation}
\kappa_{2} (A_{2D}) = \frac {\lambda_{n-1,n-1}} {\lambda_{1,1}} = \frac {\sin^{2} (\frac {\pi} {2} - \frac {\pi h} {2})} {\sin^{2} (\frac {\pi h} {2})} = \frac {\cos^{2} (\frac {\pi h} {2})} {\sin^{2} (\frac {\pi h} {2})}
\end{equation}
...
Gleichungssysteme, die partielle Differentialgleichungen lösen, können sehr groß werden. Aus diesem Grund sind direkte Verfahren, wie z.B. der Gauß-Algorithmus oder die LR-Zerlegung nicht geeignet. Ihr Rechenaufwand beläuft sich im Allgemeinen auf $\mathcal{O}(n^{3})$ und ist dadurch zu langsam.\\
Wesentlich besser geeignet für diese Problemstellung sind iterative Verfahren. Sie zeichnen sich durch eine schnelle Konvergenz und einen geringen Rechenaufwand aus, falls sie konvergieren.

\section{Grundbegriffe}

\subsection{Definition (Spektralradius)}\label{s.Spektralradius}

Sei $\mat{A} \in \mathbb{R}^{n \times n}$. Dann gilt:
\begin{equation}
\rho(A) := \max_{1 \le i \le n} | \lambda_{i} |
\end{equation}
Ist $\mat{A}$ symmetrisch so gilt auch: $\| A \|_{2} = \rho(A)$

\subsection{Definition (Iterationsmatrix)}
...
\end{equation}
wobei die Iterationsvorschrift gegeben ist durch:
\begin{equation}
u^{k+1} := (\mat{Id} - \mat{C} \mat{A})u^{k} + \mat{C} f \textnormal{ für k = 0,1,2,...}\label{eq.Iteratives Verfahren}
\end{equation}

\subsection{Satz (Konvergenz iterativer Verfahren)}
...
\subsection{Definition (Residuum und Fehler)}

Sei $u^{*} \in \mathbb{R}^{n}$ die exakte Lösung des linearen Gleichungssystems $Au = f$. Sei außerdem für $k = 0,1,...$  $u^{k} \in \mathbb{R}^{n}$ die Approximation der Lösung im $k-ten$ Iterationsschritt. Dann gilt für das Residuum:
\begin{equation}
...
\end{equation}
Die letzte Gleichung nennen wir Residuumsgleichung:

\section{Das Jacobi-Verfahren (Gesamtschrittverfahren)}\label{s.Das Jacobi-Iterationsverfahren}

Für die Beispiele gilt stets: $n := (N-1) \cdot (N-1)$, wobei $(N-1)$ die Gitterweite in x- und y-Richtung darstellt.
...
In jedem Schritt zur Berechnung von $u^{k}$ muss hier die Information seines Vorgängers $u^{k-1}$ bekannt sein. Der Rechenaufwand pro Iterationsschritt beträgt $\mathcal{O}(n^{2})$ und entspricht somit einer Matrix-Vektor-Multiplikation.

\subsection{Satz (Iterationsmatrix des Jacobi-Verfahrens)}
...
&\overset{(\mat{L}+\mat{U})=(\mat{D}-\mat{A})}{=}& D^{-1}(D-A)u^{k}+D^{-1}f \\
&=& (Id-D^{-1}A)u^{k} + D^{-1}f
\end{eqnarray}
$\Longrightarrow$ Beh.

\subsection{Satz (Eigenwerte der Jacobi-Iterationsmatrix bzgl. $\mat{A}_{2D}$)}

Für die Eigenwerte der Iterationsmatrix $\mat{T}_{J} := (\mat{Id} - \mat{D}^{-1}\mat{A}_{2D}$ gilt:
\begin{equation}
\lambda_{i,j}(\mat{T}_{J}) = 1 - \frac {4} {h^{2}} \left(\sin^{2}(\frac {i \pi h} {2}) + \sin^{2}(\frac {j \pi h} {2}) \right) \textnormal{ für } 1 \le i,j \le n-1\label{eq.Jacobi-Eigenwerte}
\end{equation}

\textbf{Beweis:}\label{b.EW Jacobi}
Für $\mat{D} = \frac {4} {h^{2}} \mat{Id}$ folgt $\mat{D}^{-1} = \frac {h^{2}} {4} \mat{Id}$. Für die Iterationsmatrix $\mat{T}_{J}$ angewandt auf einen Vektor $u$ gilt:
\begin{eqnarray}
Tu = (Id - D^{-1}A)u = Id u - D^{-1} \underbrace{Au}_{=\lambda(A) u} = - \frac {h^{2}} {4} Id \lambda(A) u =
(1 - \frac {h^{2}} {4} \lambda(A)) u
\end{eqnarray}
Die Eigenwerte von $\mat{T}$ lassen sich also einfach durch $(1 - \lambda(A)$ berechen. Somit folgt aus \autoref{eq.Eigenwerte}:
\begin{equation}
\lambda_{i,j}(\mat{T}_{J}) = 1 - \frac {4} {h^{2}} \left(\sin^{2}(\frac {i \pi h} {2}) + \sin^{2}(\frac {j \pi h} {2}) \right) \textnormal{ für } 1 \le i,j \le n-1
\end{equation}

\subsection{Lemma (Spektralradius der Jacobi-Iterationsmatrix bzgl. $\mat{A}_{2D}$)}
...
\begin{eqnarray}
\rho(Id-D^{-1}A_{2D}) = \max_{1 \le i,j \le n} | \lambda_{i} | = \max_{1 \le i,j \le n} | 1 - \frac {4} {h^{2}} \left(\sin^{2}(\frac {i \pi h} {2}) + \sin^{2}(\frac {j \pi h} {2} \right) | &=& \\
1 - 2\sin^{2} (\frac {\pi h} {2}) =\cos (\pi h) \overset{\textnormal{mit Taylorentwicklung}}{\approx} 1 - \frac {1} {2} \pi^{2} h^{2}
\end{eqnarray}
...
Für i = 1,...,N-1 und für j = 1,...,N-1:
\begin{equation}
u^{k+1}_{i,j} =  \frac {1} {a_{i,i}} (f_{i,j} - u^{k}_{i-1,j} + u^{k}_{i+1,j} + u^{k}_{i,j-1} + u^{k}_{i,j+1})
\end{equation}

Der Rechenaufwand pro Iterationsschritt beträgt lediglich $\mathcal{O}((N-1) \cdot (N-1)) = \mathcal{O}(n)$ Schritte.
...
&=& u^{k} - D^{-1}Au^{k} + D^{-1}f \\
&=& u^{k} + D^{-1} \underbrace{= r^{k}}_{(f - Au^{k})}
\end{eqnarray}
...
&=& u^{k} - \omega D^{-1} Au^{k} + \omega D^{-1} = (Id - \omega D^{-1} A)u^{k} + \omega D^{-1}f \notag \\
&=& (Id - \omega Id + \omega Id - \omega D^{-1} A)u^{k} + \omega D^{-1}f \notag \\
&=& (Id - \omega Id) + (\omega Id - \omega D^{-1} A)u^{k} + \omega D^{-1}f \notag \\
&=& (1 - \omega)Id + \omega(Id - D^{-1}A) u^{k} + \omega D^{-1}f\label{eq.Jacobi-Relaxation}
\end{eqnarray}
\autoref{eq.Jacobi-Relaxation} ist die Iterationsvorschrift für das Jacobi-Relaxationsverfahren. Die Iterationsmatrix ist gegeben durch:
\begin{equation}
...
\subsection{Algorithmus des Jacobi-Relaxations-Verfahrens}
...
\subsection{Algorithmus des Jacobi-Relaxations-Verfahrens für $\mat{A}_{2D}$}
...
\subsection{Satz (Eigenwerte des Jacobi-Relaxationsverfahren bzgl. $\mat{A}_{2D}$)}

Für die Eigenwerte von $\mat{T}_{J_{\omega}}$ gilt:
\begin{equation}
\lambda_{i,j}(\mat{T}_{J}) = 1 - \frac {4 \omega} {h^{2}} \left(\sin^{2}(\frac {i \pi h} {2}) + \sin^{2}(\frac {j \pi h} {2}) \right) \textnormal{ für } 1 \le i,j \le n-1\label{eq.Jacobi-Eigenwerte}
\end{equation}

\textbf{Beweis:}
Analog zu \autoref{b.EW Jacobi}.

\subsection{Lemma (Spektralradius der Jacobi-Relaxations-Matrix bzgl. $\mat{A}_{2D}$)}
...
\subsection{Satz (Konvergenz des CG-Algorithmus) [DahmenReusken]}
